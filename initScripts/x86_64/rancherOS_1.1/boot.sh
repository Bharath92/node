...
# Main directories
readonly SHIPPABLE_DIR="/home/shippable"
...
export_envs() {
  ...
  export REQKICK_DIR="/home/shippable/reqKick/reqKick-$SHIPPABLE_RELEASE_VERSION"
...
setup_mounts() {
  local docker_client_location=$(which docker)
  export IS_DOCKER_LEGACY=false
...
remove_genexec() {
  ...
  if [ ! -z "$running_container_ids" ]; then
    docker rm -fv $running_container_ids || true
  fi
}
...
remove_reqKick() {
  ...
  sudo ros service list | ( grep -o "^$REQKICK_SERVICE_NAME_PATTERN" || true ) | while read -r service; do
    sudo ros service stop $service stop || true
    sudo ros service stop $service disable || true
    sudo rm -rf /var/lib/rancher/conf/$REQKICK_SERVICE_NAME_PATTERN*
  done
}
...
boot_reqKick() {
  ...
  local reqKick_config_file=/var/lib/rancher/conf/shippable-reqKick-compose.yml
  ...
  sed -i "s#{{SHIPPABLE_RELEASE_VERSION}}#$SHIPPABLE_RELEASE_VERSION#g" $reqKick_config_file

  sudo ros service $reqKick_config_file enable
  sudo ros service $reqKick_service_name start

  {
    echo "Checking if $reqKick_service_name is active"
    local reqKick_status=$(sudo ros service list | grep $REQKICK_SERVICE_NAME_PATTERN | awk '{ print $1 }')
    [ "$reqKick_status" == "enabled" ] && echo "$reqKick_service_name is enabled"
  } ||
  {
    echo "$reqKick_service_name failed to start"
    sudo ros service logs $reqKick_service_name || true
    exit 1
  }
}
